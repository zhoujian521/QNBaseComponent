...
- (NSString *)USStringFormat {
    ...
    NSInteger legth = self.length;
    NSInteger bet = (legth - 1)/3;
    for (int i = 0; i < bet; i++) {
        NSString *subStr = [self substringWithRange:NSMakeRange(self.length - 3 * (i+1), 3)];
        [strArr addObject:subStr];
    }
    NSString *subStr = [self substringWithRange:NSMakeRange(0, self.length - 3*bet)];
    [strArr addObject:subStr];
    
    NSMutableArray *newArr = [NSMutableArray array];
    for (NSInteger i = strArr.count; i > 0; i--) {
        [newArr addObject:[strArr objectAtIndex:(i - 1)]];
    }
...
{
    if ([self characterAtIndex:0] <= '9' && [self characterAtIndex:0] >= '0') {
...
{
    if ([self firstCharIsInteger]) {
        
        return NO;
    }
    
    if ([self characterAtIndex:0] <= 'z' && [self characterAtIndex:0] >= 'a') {
